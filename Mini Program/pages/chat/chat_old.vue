				...
				<view class="message"
					...
					<view class="message-content">
						...
						<template v-else-if="msg.type === 'image'">
							...
							<view v-else-if="msg.role === 'user'" class="answer-section">
								<view class="image-content">
									<view class="image-container" :style="{ position: 'relative', width: '100%' }">
										<image :src="getImageUrlForUser(msg.serverPath)" class="chat-image" mode="widthFix"
											:style="{
						            width: '100%',
						            height: '100%'}" @click="previewImage(msg.localPath || getImageUrlForUser(msg.serverPath))"
											@error="onImageError" />
									</view>
									<text class="image-text" style="padding: 20px;">{{ msg.content }}</text>
								</view>
		...
		methods: {
			...
			getThumbnailUrl(serverPath) {
				...
				if (serverPath.startsWith('uploads/')) {
					serverPath =  `${this.config.server.httpUrl}/api/files/path?filePath=${encodeURIComponent(serverPath)}&thumbnail=true`;
				}
			...
			getImageUrl(serverPath) {
				...
				// 如果是相对路径，转换为服务器访问URL
				if (serverPath.startsWith('uploads/')) {
					serverPath =  `${this.config.server.httpUrl}/api/files/path?filePath=${encodeURIComponent(serverPath)}`;
				}
				serverPath = '/static/images/blue_ceramic.png';
				return serverPath;
			},
			
			getImageUrlForUser(serverPath) {
				if (!serverPath) return '';
				// 如果是相对路径，转换为服务器访问URL
				if (serverPath.startsWith('uploads/')) {
					serverPath =  `${this.config.server.httpUrl}/api/files/path/user?filePath=${encodeURIComponent(serverPath)}`;
				}
...
<style>
	page {
		...
		background: linear-gradient(to bottom, #A1C6B7 0%, #ffffff 100%);
